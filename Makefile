...
go: ${GOPATH}/bin/protoc-gen-go
	...
	go run generator/gen_errors.go -- errors/errors.yml errors.go
...
ifeq ($(PYTHON_GEN_INIT),yes)
	...
	echo 'import os' >> $(PYTHON_PREFIX)/dynagatewaytypes/__init__.py
	echo 'sys.path.append(os.path.dirname(os.path.dirname(__file__)))' >> $(PYTHON_PREFIX)/dynagatewaytypes/__init__.py
endif
